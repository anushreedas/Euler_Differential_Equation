...
This file contains the euler functions and the linear algebra package
mentioned in the assignment specification
...
def euler1(h,tb,x0,F):
    """
    Solves the differential equation using Eulers method
    :param h: step value
    :param tb: bound for t
    :param A: action to perform every loop
...
def euler(h, tb, A, x0, F):
    """
    Generalization of euler1 that supports vectors of x0 and F
    :param h: step value
    :param tb: bound for t
    :param A: action to perform every loop
    :param x0: vector of initial values of x
    ...
    while t <= tb:
        ...
        x = [xi+(h * fi) for (xi,fi) in zip(x,f)]
        t = t + h
...
def eulerCurry(h):
    """
     Curries all the arguments of euler
    """
    def p(tb):
        def q(A):
            def r(x0):
